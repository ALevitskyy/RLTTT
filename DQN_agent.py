...
class DQNagent:
    def network_init(self,input_dim,layer_1,layer_2):
        ...
        self.training_model.add(Dense(layer_1, input_dim=input_dim,W_regularizer=elastic_net))
        ...
        self.training_model.compile(loss='mse', optimizer = 'rmsprop', metrics=['accuracy'])
...
#This will be used for action replay
        ...
        self.Qpath=os.path.join("saved_agents","DQNagent-"+str(self.dimension1)+
                          "-"+str(self.dimension2)+"-"+str(self.order))
    def save_DQN(self):
    ...
    def get_reward(self,game):
        if game.gamestate=="in_play":
            ...
            return -1
    def experience_replay_add(self,state,action,reward,new_state,whose_move,length):
        if len(self.experience_replay_state_action_reward)==length:
            del self.experience_replay_state_action_reward[0]
        self.experience_replay_state_action_reward.append([state,action,reward,new_state,whose_move])
    def experience_replay_sample(self,minibatch_size):
        if len(self.experience_replay_state_action_reward)>minibatch_size:
            rand_smpl = [ self.experience_replay_state_action_reward[i] for i in 
                     sorted(random.sample(range(
                             len(self.experience_replay_state_action_reward)), minibatch_size)) ]
        else:
    ...
    def update_nn(self):
        ...
        for s, a, r, _s,move in rand_smpl: #state action reward next_state
			# print _s
            if r == 0: #update the reward if it's a non terminal step. the reward is 0 for all non terminal steps
                #IF O would want min rather than max
                if move=="X":
                    r = np.max(self.training_model.predict(np.array(_s))) # gamma*Q(s', a)
                if move=="O":
    ...
    def train_with_itself(self,number_of_episode_games):
        ...
        for i in range(number_of_episode_games):
            ...
            while game.gamestate=="in_play":
                ...
                self.experience_replay_add(old_state,self.action_decode(action,whose_move),
                                           r,new_state,game.whose_move,self.ER_mem_size)
