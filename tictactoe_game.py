...
class tictactoe:
    ...
    def check_gamestate(self,move_code,whose_move):
        down_diagonal=[[move_code[0]+i,move_code[1]+i] for i in range(-self.order+1,
                       self.order) if self.eligble_point([move_code[0]+i,move_code[1]+i])]
        up_diagonal=[[move_code[0]-i,move_code[1]+i] for i in range(-self.order+1,
                       self.order) if self.eligble_point([move_code[0]-i,move_code[1]+i])]
        vertical=[[move_code[0]+i,move_code[1]] for i in range(-self.order+1,
                       self.order) if self.eligble_point([move_code[0]+i,move_code[1]])]
        horizontal=[[move_code[0],move_code[1]+i] for i in range(-self.order+1,
                       self.order) if self.eligble_point([move_code[0],move_code[1]+i])]
        down_diagonal=[down_diagonal[i:(i+self.order)] for i in range(0,
                       len(down_diagonal)-self.order+1)]
        up_diagonal=[up_diagonal[i:(i+self.order)] for i in range(0,
                       len(up_diagonal)-self.order+1)]
        vertical=[vertical[i:(i+self.order)] for i in range(0,
                       len(vertical)-self.order+1)]
        horizontal=[horizontal[i:(i+self.order)] for i in range(0,
                       len(horizontal)-self.order+1)]
        if whose_move=='X':
            ...
            for elem3 in horizontal:
                if all(elem in self.sequence_o  for elem in elem3):
                    return("win_o")
        if len(self.sequence_o)+len(self.sequence_x)==self.dimension*self.dimension2:
            return("draw")           
        return "in_play"
